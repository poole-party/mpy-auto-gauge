...
def update(gauge, value, options):
    ...
    gauge.draw_readout(parts[0], '.' + parts[-1])
    ...
    if mdp_cur >= 0 and mdp_next >= 0:
        ...
        if lvl_next > lvl_cur or (not gauge._primary_visible[0] and mdp_next >= 0.1):
            for i in range(lvl_cur, lvl_next + 1):
                gauge.set_primary_segment(i, True)
        ...
        if lvl_next > lvl_cur or (not gauge._secondary_visible[0] and mdp_next <= -0.1):
            for i in range(lvl_cur, lvl_next + 1):
                gauge.set_secondary_segment(i, True)
        elif lvl_next < lvl_cur:
            for i in range(lvl_cur, lvl_next, -1):
                gauge.set_secondary_segment(i, False)
        gauge._bar_level = lvl_next
